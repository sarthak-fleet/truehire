...
const PRERENDERED_ROOTS = [
  ...
  // pnpm workspace layout: standalone copies the .next tree under
  // apps/web/.next, not the repo-root .next. Without this Beasties
  // would silently process only the source .next/server/app and
  // OpenNext would package the un-inlined standalone HTML.
  resolve(".next/standalone/apps/web/.next/server/app"),
