...
export default async function SuggestPage({
    ...
    return (
      <main className="mx-auto max-w-2xl px-4 py-16">
        ...
        <p className="mt-3 text-sm text-stone-600">
          Pass your GitHub handle in the query string to see which axis
          would push your overall score up the most.
        </p>
  ...
  if (!score) {
    return (
      <main className="mx-auto max-w-2xl px-4 py-16">
        ...
        <p className="mt-3 text-sm text-stone-600">
          No score yet for that handle. Sign in to claim and score the profile,
          or open <Link href={`/${handle}`} className="underline">/{handle}</Link>{" "}
          to seed the ingest.
        </p>
      </main>
    );
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-12">
      ...
      <p className="mt-3 text-sm text-stone-600">
        Current overall: <span className="font-medium tabular-nums">{score.overall}</span>.
        The biggest weighted gap is on <strong>{top.label}</strong> — pushing
        it up has the most leverage on your overall.
      </p>
      ...
      <p className="mt-8 text-xs text-stone-500">
        ...
        {SCORING_CAPS.breadthRepos} repos, recognition ≤ {SCORING_CAPS.recognition}{" "}
        (log scale). Recognition freshness half-life:{" "}
        {SCORING_HALF_LIVES.recognitionFreshnessMonths} months.
      </p>
    </main>
  );
}
