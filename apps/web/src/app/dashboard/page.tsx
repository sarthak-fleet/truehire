...
export default async function DashboardPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-6 py-10">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-1 text-[14px] text-[var(--muted)]">
            Your TrueHire profile is {score ? 'live' : 'being built'}. Share the link in
            applications, or embed it on your site.
          </p>
          <div className="mt-4 flex gap-3">
            <Link href="/recruiter/pipelines">
              <Button
                variant="ghost"
                size="sm"
                className="h-7 px-2 text-[11px] font-bold uppercase tracking-wider text-[var(--accent)] border border-[var(--accent)] hover:bg-[var(--accent)] hover:text-[var(--accent-contrast)]"
              >
                Recruiter Tools
              </Button>
            </Link>
      ...
      {isScoring ? (
        <IngestBootstrapper hasScore={!!score} ingestStatus={user.ingestStatus} />
        ...
        <Card className="mt-10 border-[color:color-mix(in_srgb,var(--warn)_40%,var(--border))]">
          <CardBody className="p-8">
            ...
            <p className="mt-2 text-sm text-[var(--muted)]">
              Your last GitHub ingest didn’t finish. Try refreshing — if it persists, check that
              your token has <code>public_repo</code> scope.
            </p>
            <div className="mt-4">
              <RefreshButton />
            </div>
          ...
          <div className="mt-10 grid grid-cols-1 gap-6 lg:grid-cols-[auto_1fr]">
            <Card className="flex flex-col items-center justify-center p-8">
              ...
              <div className="mt-5 text-center">
                <Badge tone="outline">Refreshes every 7 days · manual 1×/day</Badge>
              </div>
            </Card>
