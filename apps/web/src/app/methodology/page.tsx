...
export default function MethodologyPage() {
  ...
  const weightSum = (Object.values(weights) as number[]).reduce(
    (a, b) => a + b,
    0,
  );
  const sortedAxes = (Object.keys(weights) as Axis[]).sort(
    ...
    <main className="mx-auto max-w-3xl px-4 py-12">
      <header className="mb-10">
        ...
        <p className="mt-3 text-sm text-stone-600">
          The TrueHire score is derived from verified GitHub activity. There are
          no user-editable fields. Every number below is imported live from{" "}
        ...
        <Card>
          ...
          <CardBody>
            <p className="text-sm text-stone-600">
              Each axis is computed independently on a 0–100 scale, then
              weighted into the overall score. Weights sum to{" "}
      ...
      <section className="mt-10">
        <Card>
          ...
          <CardBody>
            <p className="text-sm text-stone-600">
              Independent of the GitHub composite. Recruiters and former
              employers can confirm work history via a verification link
              (see <Link className="underline" href="/verify">/verify</Link>);
              confirmed entries add a small bonus to the overall score.
              Unconfirmed entries are shown on the profile but contribute
              nothing.
            </p>
          </CardBody>
        </Card>
