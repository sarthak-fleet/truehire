# Archived: PRD.md v0.1 (draft) — 2026-04-24

**Preserved per documentation preferences (Claude.md + Fleet AGENTS.md):** Shorter pages (target 150-300 lines), DRY across docs, journey + learning artifacts as first-class outputs, preserve everything when consolidating (prefer `docs/archive/<name>.md` over deletion, keep git rename history).

**Context of archive:** Original full scoping document (pre-MVP) moved here on 2026-06-13 as part of finishing all PRDs / docs sync. The long catch-all format (368 lines) violated shorter-pages guidance and had become stale relative to shipped features (Signal 2 scaffold, recruiter-proof surfaces, craft axis, public exports, etc.).

**See instead:**
- `PRD.md` (current v0.2) — high-level vision, problem, phases, and pointers to canonical homes.
- `PROJECT_STATUS.md` — single source of truth for what is Done / Planned Next / Deferred.
- `docs/` — focused topic pages (signal-1, signal-2, recruiter-proof-tools, etc.).
- `plans/` — finished implementation plans for extensions.
- `apps/web/src/app/methodology/page.tsx` + `packages/core/src/scoring/score.ts` — live authoritative scoring details (no drift).

History retained for reference and git.
